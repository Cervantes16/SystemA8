...
const API_URL = 'http://localhost:3000/api'; // Base URL without /salidas to match backend routes
...
export const getTallasDisponibles = async () => {
  try {
    ...
    throw new Error(error.response?.data?.error || 'Error al obtener las tallas');
  }
};
...
export const getCajasPorTalla = async (tallaid: number) => {
  try {
    ...
    const response = await axios.get(`${API_URL}/salidas/cajas/talla/${tallaid}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return response.data;
  } catch (error: any) {
    console.error('Error fetching cajas por talla:', error);
    throw new Error(error.response?.data?.error || 'Error al obtener las cajas por talla');
  }
};
...
export const getCajasDisponiblesPorTalla = async (tallaid: number) => {
  try {
    ...
    const response = await axios.get(`${API_URL}/salidas/cajas/talla/${tallaid}/count`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return response.data.count;
  } catch (error: any) {
    console.error('Error fetching count of cajas por talla:', error);
    throw new Error(error.response?.data?.error || 'Error al obtener el conteo de cajas disponibles');
  }
};
...
export const createSalida = async (data: FormData) => {
  try {
    ...
    throw new Error(error.response?.data?.error || 'Error al registrar la salida');
  }
};
...
export const getCajasForSolicitud = async (solicitudid: number) => {
  try {
    ...
    const response = await axios.get(`${API_URL}/solicitudes/${solicitudid}/cajas`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return response.data;
  } catch (error: any) {
    console.error('Error fetching cajas for solicitud:', error);
    throw new Error(error.response?.data?.error || 'Error al obtener cajas para la solicitud');
  }
};
...
export const approveSolicitud = async (solicitudid: number) => {
  try {
    ...
    const response = await axios.put(`${API_URL}/solicitudes/${solicitudid}/approve`, {}, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return response.data;
  } catch (error: any) {
    console.error('Error approving solicitud:', error);
    throw new Error(error.response?.data?.error || 'Error al aprobar la solicitud');
  }
};